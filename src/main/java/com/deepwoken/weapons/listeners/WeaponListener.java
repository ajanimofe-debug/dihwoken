...
public class WeaponListener implements Listener {
    ...
    private final Map<UUID, Long>    pyreFlameCd   = new HashMap<>(); // Lifelord's Blaze 4s cooldown
    ...
    public void onRightClick(PlayerInteractEvent e) {
        ...
        if (wid == null) return;
        e.setCancelled(true);
        if (wid.equals("flamewall") && flameDisabled.contains(p.getUniqueId())) { p.sendActionBar(Component.text("🔥 Overheated!",NamedTextColor.RED)); return; }
        int cdMs = switch(wid) { case "red_death" -> 8000; case "boltcrusher","crypt_blade","stormseye","hero_blade_thunder","yselys_pyre_keeper" -> 10000; default -> 9000; };
    ...
    public void onSwitch(PlayerItemHeldEvent e) {
        ...
        // Give Verdant to offhand when switching to Pyre Keeper
    ...
    private void m1Stormseye(Player p, LivingEntity t) {
        ...
        // Rosen's Fakeout — aerial + close range → teleport behind
        ...
        // Track hits for Medallion Shockwave
        ...
        // Every 3rd hit — Medallion Shockwave
        if (hits>=3) {
    ...
    private void m1PyreKeeper(Player p, LivingEntity t) {
        // Lifelord's Blaze — green flames every 4 seconds
        long now = System.currentTimeMillis();
        UUID uuid = p.getUniqueId();
    ...
    private void m1Flamewall(Player p, LivingEntity t) {
        ...
        t.setFireTicks(40+(h*8)); t.addPotionEffect(new PotionEffect(PotionEffectType.WEAKNESS,60,0,false,true,true));
    ...
    private void critStormseye(Player p) {
        ...
        p.addPotionEffect(new PotionEffect(PotionEffectType.RESISTANCE,40,4,false,true,true)); // I-frames
        sound(p.getLocation(),Sound.ENTITY_LIGHTNING_BOLT_THUNDER,0.5f,1.5f);
        // Circle enemy firing 3 shots
        for(int i=0;i<3;i++){
            final int si=i;
            new BukkitRunnable(){@Override public void run(){
                ...
                target.damage(si==2?10:7,p); // last shot does more + Shocked
                if(si==2){
                    slow(target,3,100);
                    target.addPotionEffect(new PotionEffect(PotionEffectType.WEAKNESS,80,1,false,true,true));
                    target.getWorld().strikeLightningEffect(target.getLocation());
                    p.sendActionBar(Component.text("⚡ SHOCKED!",NamedTextColor.YELLOW));
                }
    ...
    private void critPyreKeeper(Player p) {
        if (!p.isOnGround() && !p.isSprinting() && !p.isSneaking()) {
            // ── Aerial: raise sword → downchop → guardbreak → explosion ───────
            p.sendActionBar(Component.text("🔥 Aerial Downchop!",NamedTextColor.GOLD));
            p.setVelocity(new Vector(0,0.5,0));
            particles(p.getLocation(),Particle.FLAME,15);
            new BukkitRunnable(){int t=0; @Override public void run(){t++;
                if(t==8){p.setVelocity(new Vector(0,-3,0));}
                if(t>=8){p.getNearbyEntities(2.5,2.5,2.5).forEach(en->{ if(en instanceof LivingEntity le&&en!=p){
                    le.damage(12,p); le.setFireTicks(80); kb(p,le,2,0.5); slow(le,2,40);
                    particles(le.getLocation(),Particle.FLAME,20);
                }});}
                if(p.isOnGround()&&t>=8){
                    // Explosion on land
                    p.getLocation().getNearbyLivingEntities(4).forEach(le->{ if(le!=p){le.damage(8,p);le.setFireTicks(100);kb(p,le,2.5,1.0);particles(le.getLocation(),Particle.FLAME,25);} });
                    p.getWorld().createExplosion(p.getLocation(),0f,false,false);
                    sound(p.getLocation(),Sound.ENTITY_GENERIC_EXPLODE,1f,0.7f);
                    cancel();
                }
                if(t>=40) cancel();
            ...
            // ── Sliding: flip forward → constant slicing ───────────────────────
            ...
            new BukkitRunnable(){int t=0; @Override public void run(){t++;
                p.setVelocity(new Vector(dir.getX(),0.3,dir.getZ()));
                p.getNearbyEntities(2,2,2).forEach(en->{ if(en instanceof LivingEntity le&&en!=p){
                    le.damage(4,p); le.setFireTicks(40); particles(le.getLocation(),Particle.FLAME,8);
                }});
                particles(p.getLocation(),Particle.FLAME,5);
                if(t>=12) cancel();
            }}.runTaskTimer(plugin,0L,1L);

        } else if (p.isSprinting()) {
            // ── Running: rush → knockdown → diagonal split ────────────────────
            ...
            particles(p.getLocation(),Particle.FLAME,15);
            new BukkitRunnable(){int t=0; @Override public void run(){t++;
                p.getNearbyEntities(2.5,2.5,2.5).forEach(en->{ if(en instanceof LivingEntity le&&en!=p){
                    le.damage(10,p); le.setFireTicks(80);
                    kb(p,le,2,0.5); // knockdown
                    slow(le,2,40);
                    particles(le.getLocation(),Particle.FLAME,20);
                    sound(le.getLocation(),Sound.ENTITY_PLAYER_ATTACK_CRIT,1f,0.8f);
                    // Follow-up diagonal split
                    new BukkitRunnable(){@Override public void run(){
                        le.damage(8,p); le.setFireTicks(60);
                        Vector split=p.getLocation().subtract(le.getLocation()).toVector().normalize().multiply(-2); split.setY(0.3); le.setVelocity(split);
                        particles(le.getLocation(),Particle.FLAME,15);
                    }}.runTaskLater(plugin,8L);
                }});
                if(t>=8) cancel();
            ...
            // ── Crouching: swift slash → flip → lighter slash + speed ─────────
            ...
            p.getNearbyEntities(3,3,3).forEach(en->{ if(en instanceof LivingEntity le&&en!=p){
                le.damage(8,p); le.setFireTicks(60); particles(le.getLocation(),Particle.FLAME,12);
                sound(le.getLocation(),Sound.ENTITY_PLAYER_ATTACK_SWEEP,0.8f,1.3f);
            }});
            // Flip + lighter slash
            new BukkitRunnable(){@Override public void run(){
                p.setVelocity(new Vector(0,0.8,0)); // flip
                particles(p.getLocation(),Particle.FLAME,10);
                new BukkitRunnable(){@Override public void run(){
                    p.getNearbyEntities(3.5,3.5,3.5).forEach(en->{ if(en instanceof LivingEntity le&&en!=p){
                        le.damage(6,p); le.setFireTicks(40); particles(le.getLocation(),Particle.FLAME,8);
                    }});
                    p.addPotionEffect(new PotionEffect(PotionEffectType.SPEED,60,1,false,true,true));
                    p.sendActionBar(Component.text("🔥 Speed Boost!",NamedTextColor.GOLD));
                }}.runTaskLater(plugin,8L);
            }}.runTaskLater(plugin,5L);

        } else {
            // ── Standing: slow slash → 2 fast slashes → speed boost ───────────
            ...
            // Slow slash
            new BukkitRunnable(){@Override public void run(){
                p.getNearbyEntities(3,3,3).forEach(en->{ if(en instanceof LivingEntity le&&en!=p){le.damage(8,p);le.setFireTicks(60);particles(le.getLocation(),Particle.FLAME,12);} });
                sound(p.getLocation(),Sound.ENTITY_PLAYER_ATTACK_SWEEP,0.8f,0.8f);
            }}.runTaskLater(plugin,8L);
            // Fast slash 1
            new BukkitRunnable(){@Override public void run(){
                p.getNearbyEntities(3,3,3).forEach(en->{ if(en instanceof LivingEntity le&&en!=p){le.damage(6,p);le.setFireTicks(40);particles(le.getLocation(),Particle.FLAME,8);} });
                sound(p.getLocation(),Sound.ENTITY_PLAYER_ATTACK_SWEEP,0.8f,1.0f);
            }}.runTaskLater(plugin,16L);
            // Fast slash 2 + speed boost
            new BukkitRunnable(){@Override public void run(){
                p.getNearbyEntities(3,3,3).forEach(en->{ if(en instanceof LivingEntity le&&en!=p){le.damage(6,p);le.setFireTicks(40);particles(le.getLocation(),Particle.FLAME,8);} });
                sound(p.getLocation(),Sound.ENTITY_PLAYER_ATTACK_SWEEP,0.8f,1.2f);
                p.addPotionEffect(new PotionEffect(PotionEffectType.SPEED,80,1,false,true,true));
                p.sendActionBar(Component.text("🔥 Speed Boost!",NamedTextColor.GOLD));
    ...
    private void critRailblade(Player p) {
        if(!p.isOnGround()){
            ...
            p.setVelocity(new Vector(0,0.4,0)); particles(p.getLocation(),Particle.CRIT,15); sound(p.getLocation(),Sound.ENTITY_PHANTOM_FLAP,0.8f,2f);
            ...
            particles(p.getLocation(),Particle.CRIT,10); sound(p.getLocation(),Sound.ENTITY_PLAYER_ATTACK_SWEEP,0.8f,0.6f);
            new BukkitRunnable(){@Override public void run(){ p.getNearbyEntities(3,3,3).forEach(en->{ if(en instanceof LivingEntity le&&en!=p){le.damage(6,p);kb(p,le,2.5,0.5);slow(le,2,30);particles(le.getLocation(),Particle.SWEEP_ATTACK,5);sound(le.getLocation(),Sound.ENTITY_PLAYER_ATTACK_KNOCKBACK,1f,1f);} }); }}.runTaskLater(plugin,8L);
    ...
    private void critKyrsewinter(Player p) {
        if(p.isSprinting()){
            ...
            particles(p.getLocation(),Particle.SNOWFLAKE,20); sound(p.getLocation(),Sound.ENTITY_PLAYER_ATTACK_SWEEP,0.8f,0.5f);
            ...
            new BukkitRunnable(){int tick=0; @Override public void run(){tick++;
                ...
                if(tick==22){p.getLocation().getNearbyLivingEntities(4).forEach(le->{ if(le!=p){le.damage(8,p);slow(le,3,100);kb(p,le,2,0.3);particles(le.getLocation(),Particle.SNOWFLAKE,25);} });particles(p.getLocation(),Particle.SNOWFLAKE,40);sound(p.getLocation(),Sound.ENTITY_GENERIC_EXPLODE,0.6f,1.8f);}
                if(tick>=25) cancel();
            }}.runTaskTimer(plugin,0L,1L);
        } else {
            p.sendActionBar(Component.text("❄ Ice Poke!",NamedTextColor.AQUA));
            particles(p.getLocation(),Particle.SNOWFLAKE,10); sound(p.getLocation(),Sound.ENTITY_PLAYER_ATTACK_SWEEP,0.8f,0.7f);
            new BukkitRunnable(){@Override public void run(){
                p.getNearbyEntities(3.5,2,3.5).forEach(en->{ if(en instanceof LivingEntity le&&en!=p){le.damage(10,p);kb(p,le,2.5,0.5);bleed(le,p,3);particles(le.getLocation(),Particle.SNOWFLAKE,20);sound(le.getLocation(),Sound.BLOCK_GLASS_BREAK,0.8f,1.2f);
                    Location bl=le.getLocation().clone();
                    new BukkitRunnable(){@Override public void run(){bl.getWorld().createExplosion(bl,0f,false,false);bl.getNearbyLivingEntities(3).forEach(e2->{ if(e2!=p){e2.damage(8,p);slow(e2,3,80);e2.setVelocity(new Vector(0,2,0));particles(e2.getLocation(),Particle.SNOWFLAKE,30);} });particles(bl,Particle.SNOWFLAKE,50);sound(bl,Sound.ENTITY_GENERIC_EXPLODE,0.7f,1.6f);}}.runTaskLater(plugin,20L);
                }});
    ...
    private void critDeepspindle(Player p) {
        if(p.isSprinting()){
            ...
            if(target!=null){for(int i=0;i<2;i++){final int bi=i; new BukkitRunnable(){@Override public void run(){particles(target.getLocation(),Particle.ASH,15);particles(target.getLocation(),Particle.TOTEM_OF_UNDYING,10);target.damage(10,p);wither(target,2,80);sound(target.getLocation(),Sound.ENTITY_WITHER_AMBIENT,0.6f,1.5f);}}.runTaskLater(plugin,(long)(bi*8+5));}}
            ...
            new BukkitRunnable(){@Override public void run(){target.damage(8,p);kb(p,target,1,0.2);particles(target.getLocation().add(0,1,0),Particle.TOTEM_OF_UNDYING,20);sound(target.getLocation(),Sound.BLOCK_BEACON_ACTIVATE,0.5f,0.5f);
                for(int i=0;i<7;i++){final int bi=i; new BukkitRunnable(){@Override public void run(){target.damage(3,p);wither(target,1,40);particles(target.getLocation(),Particle.ASH,8);particles(target.getLocation(),Particle.TOTEM_OF_UNDYING,5);sound(target.getLocation(),Sound.ENTITY_WITHER_AMBIENT,0.3f,1.8f);}}.runTaskLater(plugin,(long)(bi*5+5));}
            }}.runTaskLater(plugin,8L);
        }
    }
    ...
    private void critBoltcrusher(Player p) {
        if(p.isSprinting()){
            ...
            new BukkitRunnable(){@Override public void run(){p.getLocation().getNearbyLivingEntities(6).forEach(le->{ if(le!=p){le.getWorld().strikeLightningEffect(le.getLocation());le.damage(12,p);slow(le,2,80);kb(p,le,2,0.3);particles(le.getLocation(),Particle.ELECTRIC_SPARK,20);} });p.getWorld().createExplosion(p.getLocation(),0f,false,false);sound(p.getLocation(),Sound.ENTITY_GENERIC_EXPLODE,1f,0.6f);particles(p.getLocation(),Particle.ELECTRIC_SPARK,40);p.sendActionBar(Component.text("⚡ SAPPED!",NamedTextColor.YELLOW));}}.runTaskLater(plugin,12L);
        } else {
            ...
            new BukkitRunnable(){@Override public void run(){p.getNearbyEntities(3,3,3).forEach(en->{ if(en instanceof LivingEntity le&&en!=p){le.damage(14,p);slow(le,2,100);kb(p,le,3,0.6);le.getWorld().strikeLightningEffect(le.getLocation());particles(le.getLocation(),Particle.ELECTRIC_SPARK,25);sound(le.getLocation(),Sound.ENTITY_PLAYER_ATTACK_CRIT,1f,0.7f);} });p.sendActionBar(Component.text("⚡ SAPPED!",NamedTextColor.YELLOW));}}.runTaskLater(plugin,14L);
        }
    }
    ...
    private void critCryptBlade(Player p) {
        if(p.isSprinting()){
            ...
            for(int i=0;i<2;i++){new BukkitRunnable(){@Override public void run(){p.getNearbyEntities(2.5,2.5,2.5).forEach(en->{ if(en instanceof LivingEntity le&&en!=p){le.damage(5,p);particles(le.getLocation(),Particle.ASH,8);sound(le.getLocation(),Sound.ENTITY_PLAYER_ATTACK_SWEEP,0.7f,1.2f);} });}}.runTaskLater(plugin,(long)(i*5));}
            new BukkitRunnable(){@Override public void run(){particles(p.getLocation(),Particle.TOTEM_OF_UNDYING,15);sound(p.getLocation(),Sound.ENTITY_WITHER_SHOOT,0.7f,0.5f);p.getNearbyEntities(3.5,3.5,3.5).forEach(en->{ if(en instanceof LivingEntity le&&en!=p){le.damage(12,p);kb(p,le,3,0.8);wither(le,1,80);slow(le,2,60);particles(le.getLocation(),Particle.ASH,20);sound(le.getLocation(),Sound.ENTITY_PLAYER_ATTACK_CRIT,1f,0.6f);} });}}.runTaskLater(plugin,14L);
        } else {
    ...
    private void critRedDeath(Player p) {
        if(p.isSprinting()){
            ...
            for(int i=0;i<5;i++){new BukkitRunnable(){@Override public void run(){p.setVelocity(move);p.getNearbyEntities(2,2,2).forEach(en->{ if(en instanceof LivingEntity le&&en!=p){le.damage(4,p);m1Bloodfouler(p,le);particles(le.getLocation(),Particle.DRIPPING_WATER,5);sound(le.getLocation(),Sound.ENTITY_PLAYER_ATTACK_SWEEP,0.6f,1.3f);} });}}.runTaskLater(plugin,(long)(i*3));}
            new BukkitRunnable(){@Override public void run(){p.getNearbyEntities(2.5,2.5,2.5).forEach(en->{ if(en instanceof LivingEntity le&&en!=p){le.damage(8,p);m1Bloodfouler(p,le);wither(le,1,60);} });spawnBloodStakes(p);}}.runTaskLater(plugin,20L);
        } else {
    ...
    private void spawnBloodStakes(Player p) {
        ...
        for(int i=0;i<3;i++){final int si=i; double angle=(Math.PI/6)*(si-1);
            ...
            new BukkitRunnable(){int t=0; Location pos=p.getLocation().clone().add(dir); @Override public void run(){t++;pos.add(dir);particles(pos,Particle.DRIPPING_WATER,5);particles(pos,Particle.ASH,3);pos.getNearbyLivingEntities(1.2).forEach(le->{ if(le!=p){le.damage(8,p);m1Bloodfouler(p,le);wither(le,1,60);particles(le.getLocation(),Particle.TOTEM_OF_UNDYING,10);} });if(t>=10)cancel();}}.runTaskTimer(plugin,(long)(si*2),1L);
        }
    }
    ...
    private void critSoulthorn(Player p) {
        ...
        else{p.sendActionBar(Component.text("✦✦✦ TRUE HYPERARMOR!",NamedTextColor.GOLD));p.teleport(target.getLocation().add(0,3,0));p.addPotionEffect(new PotionEffect(PotionEffectType.RESISTANCE,30,4,false,true,true));for(int i=0;i<4;i++){new BukkitRunnable(){@Override public void run(){target.damage(6,p);p.setVelocity(new Vector(0,-1.5,0));particles(target.getLocation(),Particle.SWEEP_ATTACK,5);sound(target.getLocation(),Sound.ENTITY_PLAYER_ATTACK_CRIT,0.8f,1.2f);}}.runTaskLater(plugin,(long)(i*6+5));}pdc.remove(DeepwokenWeapons.SOUL_MARKS_KEY);}
    }

    private void critGalePale(Player p) {
        if(p.isSprinting()){p.sendActionBar(Component.text("🌬 Rush Sweep!",NamedTextColor.WHITE));p.setVelocity(p.getLocation().getDirection().normalize().multiply(3));new BukkitRunnable(){int t=0;@Override public void run(){t++;p.getNearbyEntities(2.5,2.5,2.5).forEach(e->{ if(e instanceof LivingEntity le&&e!=p){le.damage(10,p);slow(le,0,60);le.addPotionEffect(new PotionEffect(PotionEffectType.POISON,80,0,false,true,true));particles(le.getLocation(),Particle.CLOUD,8);} });if(t>=6)cancel();}}.runTaskTimer(plugin,0L,2L);}
        else{p.sendActionBar(Component.text("🌬 360° Sweep!",NamedTextColor.WHITE));p.getLocation().getNearbyLivingEntities(5).forEach(e->{ if(e!=p){e.damage(12,p);kb(p,e,2,0.4);slow(e,1,60);e.addPotionEffect(new PotionEffect(PotionEffectType.POISON,80,0,false,true,true));} });particles(p.getLocation(),Particle.CLOUD,40);sound(p.getLocation(),Sound.ENTITY_GENERIC_EXPLODE,0.7f,1.6f);}
    }

    private void critColdPoint(Player p) {
        LivingEntity target=nearest(p,8); if(target==null){p.sendActionBar(Component.text("No target in range.",NamedTextColor.GRAY));return;}
        ...
        target.damage(10,p);slow(target,3,120);target.addPotionEffect(new PotionEffect(PotionEffectType.BLINDNESS,40,0,false,true,true));particles(target.getLocation(),Particle.SNOWFLAKE,30);sound(target.getLocation(),Sound.BLOCK_GLASS_BREAK,1f,1.8f);
    }
    ...
    private void startBloodDot(LivingEntity t, Player p){ new BukkitRunnable(){int ticks=0;@Override public void run(){ticks++;if(!t.isValid()||t.isDead()){cancel();return;}var pdc=t.getPersistentDataContainer();int s=pdc.getOrDefault(DeepwokenWeapons.BLOOD_POISON_KEY,PersistentDataType.INTEGER,0);if(s<=0){cancel();return;}t.damage(s*0.5,p);particles(t.getLocation().add(0,1,0),Particle.DRIPPING_WATER,3);if(ticks%5==0)pdc.set(DeepwokenWeapons.BLOOD_POISON_KEY,PersistentDataType.INTEGER,Math.max(0,s-1));if(ticks>100)cancel();}}.runTaskTimer(plugin,20L,20L); }
    private void startChainPull(Player p, LivingEntity target){ Location anchor=target.getLocation().clone(); new BukkitRunnable(){int t=0;@Override public void run(){t++;if(t>60||!target.isValid()||target.isDead()){cryptChain.remove(p.getUniqueId());cancel();return;}if(target.getLocation().distanceSquared(anchor)>16){Vector pull=anchor.toVector().subtract(target.getLocation().toVector()).normalize().multiply(2);target.setVelocity(pull);particles(target.getLocation(),Particle.ASH,5);}}}.runTaskTimer(plugin,0L,2L); }
    private void triggerChainBreak(Player p, LivingEntity t){ cryptChain.remove(p.getUniqueId()); for(int i=0;i<3;i++){new BukkitRunnable(){@Override public void run(){t.damage(5,p);wither(t,2,40);particles(t.getLocation(),Particle.TOTEM_OF_UNDYING,10);}}.runTaskLater(plugin,(long)(i*5));}p.sendActionBar(Component.text("💀 Chain Broken!",NamedTextColor.DARK_GREEN));sound(t.getLocation(),Sound.ENTITY_WITHER_SHOOT,0.8f,1.5f); }
    private void handleProjectile(Snowball ball, Player owner){ new BukkitRunnable(){int t=0;@Override public void run(){t++;if(!ball.isValid()||t>60){cancel();return;}ball.getNearbyEntities(0.5,0.5,0.5).forEach(e->{ if(e instanceof LivingEntity le&&e!=owner){le.damage(12,owner);ball.getWorld().createExplosion(ball.getLocation(),0f,false,false);particles(ball.getLocation(),Particle.EXPLOSION_EMITTER,3);ball.remove();cancel();} });}}.runTaskTimer(plugin,0L,1L); }
