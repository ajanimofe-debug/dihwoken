...
public class WeaponListener implements Listener {
    ...
    // FIX: Re-entry guard prevents ALL cascade damage from onHit
    private final Set<UUID> processingHit = new HashSet<>();

    // FIX: Track plugin-fired arrows so their damage events are ignored
    ...
    // FIX: shared Random, not new every call
    ...
    // FIX: Ignore damage from plugin-fired arrows to prevent cascade
    @EventHandler(priority = EventPriority.HIGHEST)
    public void onArrowHit(EntityDamageByEntityEvent e) {
        if (!(e.getDamager() instanceof Arrow arrow)) return;
        if (pluginArrows.contains(arrow.getUniqueId())) {
            // Arrow hit registered — remove tracking, let damage through but skip weapon logic
            // We do NOT cancel — arrow damage is intentional, we just don't want onHit cascade
            pluginArrows.remove(arrow.getUniqueId());
        }
    }
    ...
    private void initPlayer(UUID u) {
        ...
        lastWeapon.put(u, "");       // FIX: always initialized
        rhythm.put(u, 0);
        heat.put(u, 0);
        resonance.put(u, 0);
        lastRhythm.put(u, 0L);
        pyreFlameCd.put(u, 0L);
        shockwaveCd.put(u, 0L);
        lastHitTime.put(u, 0L);
        lastMove.put(u, 0L);
    }
    ...
    public void onQuit(PlayerQuitEvent e) {
        ...
        pyreFlameCd.remove(u); zodiacSign.remove(u); zodiacCooldown.remove(u);
        voidMeter.remove(u); orbitCharge.remove(u); lastMove.remove(u);
    ...
    // ─── Move (Orbit passive) — FIX: null-safe ───────────────────────────────
    @EventHandler
    public void onMove(PlayerMoveEvent e) {
        if (e.getFrom().distanceSquared(e.getTo()) < 0.01) return;
        Player p = e.getPlayer();
        UUID u = p.getUniqueId();
        // FIX: use getOrDefault to avoid NPE if lastWeapon not yet set
    ...
    public void onHit(EntityDamageByEntityEvent e) {
        ...
        UUID u = p.getUniqueId();
        // FIX: re-entry guard — prevents ALL recursive/cascade onHit
        ...
        try {
            ...
            // FIX: ALWAYS remove guard even if exception thrown
            processingHit.remove(u);
        }
    }
    ...
    private void m1Kyrsewinter(Player p, LivingEntity t, int step, boolean air, boolean sprint) {
        if (air) { safeGroundTask(p, () -> p.getLocation().getNearbyLivingEntities(3).stream().limit(5).forEach(le -> { if (le != p) { le.damage(8, p); slow(le, 3, 80); } })); p.setVelocity(new Vector(0, -2.5, 0)); p.sendActionBar(Component.text("❄ Ice Drop!", NamedTextColor.AQUA)); return; }
        if (sprint) { slow(t, 3, 60); p.setVelocity(p.getLocation().getDirection().normalize().multiply(1.2)); return; }
        slow(t, 1, 40); bleed(t, p, 2);
        if (step % 4 == 3) { t.getLocation().getNearbyLivingEntities(3).stream().limit(5).forEach(le -> { if (le != p) { le.damage(6, p); slow(le, 3, 80); } }); sound(p.getLocation(), Sound.BLOCK_GLASS_BREAK, 0.8f, 1.5f); p.sendActionBar(Component.text("❄ Ice Burst!", NamedTextColor.AQUA)); }
    }

    private void m1Stormseye(Player p, LivingEntity t, int step, boolean air, boolean sprint) {
        UUID u = p.getUniqueId();
        // FIX: aerial shot does NOT increment shockwave counter
    ...
    private void m1HeroFire(Player p, LivingEntity t, int step, boolean air, boolean sprint) {
        ...
        if (step % 4 == 3) {
            ...
            // FIX: only set fire ticks on nearby — no damage to prevent cascade
            t.getLocation().getNearbyLivingEntities(3).stream().limit(4).forEach(le -> { if (le != p && le != t) le.setFireTicks(60); });
    ...
    private void m1HeroWind(Player p, LivingEntity t, int step, boolean air, boolean sprint) {
        ...
        Vector push = safeDirTo(t.getLocation(), p.getLocation()).multiply(0.3 * stacks); push.setY(0.1);
    ...
    private void m1HeroThunder(Player p, LivingEntity t, int step, boolean air, boolean sprint) {
        ...
        // FIX: overcharge does fixed 20 damage, not percentage of HP
    ...
    private void m1Railblade(Player p, LivingEntity t, int step, boolean air, boolean sprint) {
        // FIX: air M1 is own inline logic — does NOT call critRailblade
        if (air) {
            p.setVelocity(new Vector(0, 0.4, 0));
            // FIX: use boolean array for lambda capture
            boolean[] hitArr = {false};
            safeGroundTask(p, () -> { if (!hitArr[0]) { p.getNearbyEntities(2.5, 2.5, 2.5).forEach(en -> { if (en instanceof LivingEntity le && en != p && !hitArr[0]) { le.damage(14, p); fireDamage(le, p, 2, 80); kb(p, le, 1.5, 0.3); hitArr[0] = true; } }); } });
        ...
        if (step % 4 == 3) { fireDamage(t, p, 4, 100); sound(t.getLocation(), Sound.ENTITY_BLAZE_SHOOT, 0.7f, 1.2f); p.sendActionBar(Component.text("💨🔥 Flaming Blow!", NamedTextColor.GOLD)); }
    }
    ...
    private void m1RedDeath(Player p, LivingEntity t, int step, boolean air, boolean sprint) {
        ...
        long now = System.currentTimeMillis();
        if (now - critCooldowns.computeIfAbsent(u, k -> new HashMap<>()).getOrDefault("red_m1", 0L) >= 600) {
            critCooldowns.get(u).put("red_m1", now);
            applyBloodPoison(p, t);
        }
        // FIX: only set fire/poison on nearby — no damage cascade
        if (step % 4 == 3) { t.getLocation().getNearbyLivingEntities(3).stream().limit(4).forEach(le -> { if (le != p && le != t) { le.setFireTicks(60); le.addPotionEffect(new PotionEffect(PotionEffectType.POISON, 40, 0, false, true, true)); } }); p.sendActionBar(Component.text("🩸 Blood Explosion!", NamedTextColor.RED)); }
    }
    ...
    private void m1Soulthorn(Player p, LivingEntity t, int step, boolean air, boolean sprint) {
        ...
        // FIX: mark always applied regardless of teleport success
    ...
    private void m1PyreKeeper(Player p, LivingEntity t, int step, boolean air, boolean sprint) {
        UUID u = p.getUniqueId();
        if (air) { fireDamage(t, p, 3, 80); return; }
        // FIX: sprint is own logic — does NOT call crit
        if (sprint) { fireDamage(t, p, 2, 60); p.setVelocity(p.getLocation().getDirection().normalize().multiply(0.8)); return; }
        long now = System.currentTimeMillis();
        if (now - pyreFlameCd.getOrDefault(u, 0L) >= 4000) {
            pyreFlameCd.put(u, now);
            fireDamage(t, p, 3, 80);
        ...
        // FIX: only fire ticks on nearby — no damage cascade
    ...
    private void m1Bloodfouler(Player p, LivingEntity t, int step, boolean air, boolean sprint) {
        ...
        applyBloodPoison(p, t);
        // FIX: AOE only applies poison effect, not blood poison again (no double-stack)
    ...
    private void m1Boltcrusher(Player p, LivingEntity t, int step, boolean air, boolean sprint) {
        ...
        switch (step % 2) {
            ...
            // FIX: nearby entities only get lightning effect — no damage to prevent cascade
            case 1 -> { t.getNearbyEntities(4, 4, 4).stream().limit(3).forEach(en -> { if (en instanceof LivingEntity le && en != p && le != t) { le.getWorld().strikeLightningEffect(le.getLocation()); le.damage(4, p); } }); kb(p, t, 2, 0.5); p.sendActionBar(Component.text("⚡ Thunder Strike!", NamedTextColor.YELLOW)); }
        }
    }

    private void m1Hailbreaker(Player p, LivingEntity t, int step, boolean air, boolean sprint) {
        // FIX: use current hail stage for air attack
    ...
    private void m1GalePale(Player p, LivingEntity t, int step, boolean air, boolean sprint) {
        ...
        // FIX: kb called only ONCE per case
        switch (step % 2) {
    ...
    private void m1IronRequiem(Player p, LivingEntity t, EntityDamageByEntityEvent e, boolean air, boolean sprint) {
        ...
        // FIX: cap aerial multiplier to prevent one shots
        if (b > 0) {
            bullets.put(p.getUniqueId(), b - 1);
            if (air) e.setDamage(e.getDamage() * 1.3);
    ...
    private void m1Amethyst(Player p, LivingEntity t, int step, boolean air, boolean sprint) {
        ...
        if (sprint) {
            ...
            // FIX: only add resonance if teleport succeeded
            if (safe != null) { p.teleport(safe); addResonance(p, 1); }
            return;
        }
        addResonance(p, 1);
        // FIX: no damage on AOE finisher — just slow
        if (step % 4 == 3) { t.getLocation().getNearbyLivingEntities(2).stream().limit(3).forEach(le -> { if (le != p && le != t) slow(le, 1, 40); }); addResonance(p, 1); p.sendActionBar(Component.text("💎 Crystal Burst!", NamedTextColor.LIGHT_PURPLE)); }
    }
    ...
    private void m1Flamewall(Player p, LivingEntity t, int step, boolean air, boolean sprint) {
        ...
        // FIX: sprint is own logic — does NOT call critFlamewall
        ...
        // FIX: only fire ticks nearby — no damage cascade
        if (step % 3 == 2) { t.getLocation().getNearbyLivingEntities(3).stream().limit(4).forEach(le -> { if (le != p && le != t) { le.setFireTicks(80); le.addPotionEffect(new PotionEffect(PotionEffectType.WEAKNESS, 60, 0, false, true, true)); } }); p.sendActionBar(Component.text("🔥 Heat Burst!", NamedTextColor.GOLD)); }
    }
    ...
    private void m1ImBlue(Player p, LivingEntity t, int step, boolean air, boolean sprint) {
        ...
        if (air) {
            ...
            // FIX: capture rhythm AFTER update, not before
            new BukkitRunnable() {
                @Override public void run() {
                    if (!p.isOnline()) { cancel(); return; }
                    if (p.isOnGround()) {
                        int r = rhythm.getOrDefault(u, 0);
                        p.getLocation().getNearbyLivingEntities(4).stream().limit(5).forEach(le -> { if (le != p) { le.damage(4 + (r * 0.5), p); kb(p, le, 2, 0.4); slow(le, 1, 40); } });
                        cancel();
                    }
                }
            }.runTaskTimer(plugin, 0L, 1L);
            p.sendActionBar(Component.text("🌊 Tidal Crash!", NamedTextColor.AQUA)); return;
        }
    ...
    private void m1Zodiac(Player p, LivingEntity t, int step, boolean air, boolean sprint) {
        ...
        switch (sign) {
            ...
            case 1 -> {
                // FIX: Libra M1 has 2s cooldown — no more permanent resistance
                long now = System.currentTimeMillis();
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    //  CRITS — FIX: all crit damage calls are safe (no cascade since we're not in onHit)
    // ═══════════════════════════════════════════════════════════════════════════
    ...
    private void critHeroFire(Player p) { p.sendActionBar(Component.text("🔥 Flame Pillar!", NamedTextColor.RED)); LivingEntity t = nearest(p, 8); if (t == null) return; for (int i = 0; i < 3; i++) { final int fi = i; new BukkitRunnable() { @Override public void run() { if (!t.isValid() || t.isDead()) return; fireDamage(t, p, 3, 60); } }.runTaskLater(plugin, (long)(fi * 5)); } }
    ...
    private void critRedDeath(Player p) {
        if (p.isSprinting()) {
            ...
            for (int i = 0; i < 4; i++) { new BukkitRunnable() { @Override public void run() { if (!p.isOnline()) return; p.setVelocity(move); p.getNearbyEntities(2, 2, 2).forEach(en -> { if (en instanceof LivingEntity le && en != p) { le.damage(6, p); applyBloodPoison(p, le); } }); } }.runTaskLater(plugin, (long)(i * 4)); }
            new BukkitRunnable() { @Override public void run() { if (!p.isOnline()) return; p.getNearbyEntities(2.5, 2.5, 2.5).forEach(en -> { if (en instanceof LivingEntity le && en != p) { le.damage(10, p); wither(le, 1, 60); } }); spawnBloodStakes(p); } }.runTaskLater(plugin, 22L);
    ...
    private void critCryptBlade(Player p) {
        if (p.isSprinting()) {
            ...
            for (int i = 0; i < 2; i++) { new BukkitRunnable() { @Override public void run() { if (!p.isOnline()) return; p.getNearbyEntities(2.5, 2.5, 2.5).forEach(en -> { if (en instanceof LivingEntity le && en != p) le.damage(8, p); }); } }.runTaskLater(plugin, (long)(i * 5)); }
            new BukkitRunnable() { @Override public void run() { if (!p.isOnline()) return; p.getNearbyEntities(3.5, 3.5, 3.5).forEach(en -> { if (en instanceof LivingEntity le && en != p) { le.damage(16, p); kb(p, le, 3, 0.8); wither(le, 1, 80); slow(le, 2, 60); } }); } }.runTaskLater(plugin, 14L);
        } else {
    ...
    private void critPyreKeeper(Player p) {
        if (!p.isOnGround() && !p.isSprinting() && !p.isSneaking()) {
            p.sendActionBar(Component.text("🔥 Aerial Downchop!", NamedTextColor.GOLD)); p.setVelocity(new Vector(0, 0.5, 0));
            boolean[] hit = {false};
    ...
    private void critIronRequiem(Player p) {
        ...
        if (p.isSprinting()) {
            ...
            p.sendActionBar(Component.text("🔫 Spinning Fire! " + (b - shots) + "/6", NamedTextColor.GRAY));
            for (int i = 0; i < shots; i++) { new BukkitRunnable() { @Override public void run() { if (!p.isOnline()) return; LivingEntity t = nearest(p, 20); Vector dir = t != null ? safeDirTo(t.getLocation().add(0, 1, 0), p.getLocation().add(0, 1, 0)).multiply(3) : p.getLocation().getDirection().multiply(3); Arrow arrow = p.launchProjectile(Arrow.class, dir); arrow.setShooter(p); arrow.setDamage(10); pluginArrows.add(arrow.getUniqueId()); sound(p.getLocation(), Sound.ENTITY_ARROW_SHOOT, 0.8f, 1.5f); } }.runTaskLater(plugin, (long)(i * 4)); }
            ...
            if (t != null) {
                ...
                Arrow arrow = p.launchProjectile(Arrow.class, dir); arrow.setShooter(p); arrow.setDamage(16);
                pluginArrows.add(arrow.getUniqueId()); // FIX: track arrow
                ...
                sound(p.getLocation(), Sound.ENTITY_ARROW_SHOOT, 0.8f, 1.2f);
            }
            bullets.put(p.getUniqueId(), Math.min(6, b + 1));
            p.sendActionBar(Component.text("🔫 Reloaded! " + Math.min(6, b + 1) + "/6", NamedTextColor.GRAY));
        }
    }
    ...
    private void critAmethyst(Player p) {
        ...
        if (p.isSprinting()) {
            ...
            for (int i = 0; i < 5; i++) {
                ...
                // FIX: use boolean array for proper lambda capture
                boolean[] hit = {false};
                final Vector fs = shard.clone();
                new BukkitRunnable() { int t = 0; Location pos = p.getLocation().clone().add(fs); @Override public void run() { t++; pos.add(fs); if (!hit[0]) pos.getNearbyLivingEntities(1.2).forEach(le -> { if (le != p && !hit[0]) { le.damage(20, p); slow(le, 1, 40); hit[0] = true; } }); if (t >= 8) cancel(); } }.runTaskTimer(plugin, (long)(i * 2), 1L);
            }
    ...
    private void critFlamewall(Player p) {
        ...
        if (p.isSprinting()) {
            ...
            for (int col = -3; col <= 3; col++) { for (int row = 0; row <= 3; row++) { final int fc = col, fr = row; Location wallLoc = p.getLocation().add(p.getLocation().getDirection().normalize().multiply(2)).add(right.clone().multiply(fc)).add(0, fr, 0); new BukkitRunnable() { @Override public void run() { if (wallLoc.getWorld() == null) return; particles(wallLoc, Particle.FLAME, 2); wallLoc.getNearbyLivingEntities(1).forEach(le -> { if (le != p) { le.damage(3, p); fireDamage(le, p, 1, 80); } }); } }.runTaskLater(plugin, (long)((Math.abs(fc) + fr) * 2)); } }
            new BukkitRunnable() { int t = 0; @Override public void run() { t++; if (t > 60 || !p.isOnline()) { cancel(); return; } Vector r2 = p.getLocation().getDirection().rotateAroundY(Math.PI / 2).normalize(); for (int c = -3; c <= 3; c++) { for (int r = 0; r <= 3; r++) { Location wl = p.getLocation().add(p.getLocation().getDirection().normalize().multiply(2)).add(r2.clone().multiply(c)).add(0, r, 0); if (wl.getWorld() != null) { particles(wl, Particle.FLAME, 1); wl.getNearbyLivingEntities(1).forEach(le -> { if (le != p) { le.damage(1, p); fireDamage(le, p, 0, 40); } }); } } } } }.runTaskTimer(plugin, 10L, 5L);
        }
    }
    ...
    private void critZodiac(Player p) {
        ...
        switch (sign) {
            ...
            case 1 -> {
                // FIX: Libra crit now deals damage AND gives buffs
    ...
    private void critNullscapes(Player p) {
        ...
        if (p.isSprinting()) {
            ...
            LivingEntity nt = nearest(p, 15);
            Location center = nt != null ? nt.getLocation() : p.getLocation().add(p.getLocation().getDirection().multiply(5));
            ...
            // FIX: track hit entities to prevent hitting same entity multiple times across waves
    ...
    private void critOrbit(Player p) {
        ...
        if (p.isSprinting()) {
            ...
            for (int i = 0; i < shots; i++) { new BukkitRunnable() { @Override public void run() { if (!p.isOnline()) return; LivingEntity t = nearest(p, 20); if (t == null) return; Vector dir = safeDirTo(t.getLocation().add(0, 1, 0), p.getLocation().add(0, 1, 0)).multiply(3); Arrow arrow = p.launchProjectile(Arrow.class, dir); arrow.setShooter(p); arrow.setDamage(8 + (charge * 0.5)); pluginArrows.add(arrow.getUniqueId()); // FIX: track arrow sound(p.getLocation(), Sound.ENTITY_ARROW_SHOOT, 0.6f, 1.8f); } }.runTaskLater(plugin, (long)(i * 5)); }
            // FIX: charge deducted AFTER scheduling shots, not inside runTaskTimer
            ...
            new BukkitRunnable() { int t = 0; @Override public void run() { t++; if (!p.isOnline()) { cancel(); return; } if ((p.isOnGround() && t >= 3) || t > 40) { p.getLocation().getNearbyLivingEntities(7).stream().limit(7).forEach(le -> { if (le != p) { Vector pull = safeDirTo(p.getLocation(), le.getLocation()).multiply(2.5); pull.setY(0.5); le.setVelocity(pull); le.damage(16, p); slow(le, 1, 60); } }); p.getWorld().createExplosion(p.getLocation(), 0f, false, false); cancel(); } } }.runTaskTimer(plugin, 0L, 1L);
            // FIX: charge deducted immediately, not inside the runTaskTimer
            orbitCharge.put(p.getUniqueId(), Math.max(0, charge - 3));
        }
    }
    ...
    private void shiftHeroThunder(Player p) { p.sendActionBar(Component.text("⚡ Overcharge — next hit x2!", NamedTextColor.GOLD)); overcharge.put(p.getUniqueId(), true); p.addPotionEffect(new PotionEffect(PotionEffectType.STRENGTH, 60, 1, false, true, true)); sound(p.getLocation(), Sound.BLOCK_BEACON_ACTIVATE, 0.5f, 1.5f); new BukkitRunnable() { @Override public void run() { overcharge.put(p.getUniqueId(), false); } }.runTaskLater(plugin, 100L); }
    ...
    private void shiftDeepspindle(Player p) {
        ...
        // FIX: collect to list first to avoid ConcurrentModificationException
    ...
    private void shiftRedDeath(Player p) {
        ...
        // FIX: check isOnline before sending action bar
    ...
    private void shiftCryptBlade(Player p) {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            Location loc = p.getLocation().add(Math.cos(angle) * 2, 0, Math.sin(angle) * 2);
            if (loc.getWorld() == null) continue; // FIX: null world check
            Zombie z = (Zombie) loc.getWorld().spawnEntity(loc, EntityType.ZOMBIE);
            z.setCustomName("§cCrypt Servant"); z.setCustomNameVisible(true);
            new BukkitRunnable() { @Override public void run() { if (z.isValid()) z.remove(); } }.runTaskLater(plugin, 400L);
        }
    ...
    private void shiftIronRequiem(Player p) {
        ...
        for (int i = 0; i < b; i++) { new BukkitRunnable() { @Override public void run() { if (!p.isOnline()) return; LivingEntity t = nearest(p, 20); if (t == null) return; Vector dir = safeDirTo(t.getLocation().add(0, 1, 0), p.getLocation().add(0, 1, 0)).multiply(3); Arrow arrow = p.launchProjectile(Arrow.class, dir); arrow.setShooter(p); arrow.setDamage(8); pluginArrows.add(arrow.getUniqueId()); sound(p.getLocation(), Sound.ENTITY_ARROW_SHOOT, 0.6f, 1.8f); } }.runTaskLater(plugin, (long)(i * 3)); }
    ...
    private void shiftAmethyst(Player p) {
        ...
        Vector dir = p.getLocation().getDirection().normalize();
        // FIX: sequential starts (not simultaneous) + boolean array for lambda capture
        for (int i = 0; i < 8; i++) {
            double angle = (Math.PI / 3) * (i / 7.0 - 0.5);
            Vector shard = dir.clone().rotateAroundY(angle).multiply(0.5);
            final Vector fs = shard.clone();
            boolean[] hit = {false};
            new BukkitRunnable() { int t = 0; Location pos = p.getLocation().clone().add(fs); @Override public void run() { t++; if (!p.isOnline()) { cancel(); return; } pos.add(fs); if (!hit[0]) pos.getNearbyLivingEntities(1).forEach(le -> { if (le != p && !hit[0]) { le.damage(10, p); slow(le, 1, 30); hit[0] = true; } }); if (t >= 6) cancel(); } }.runTaskTimer(plugin, (long)i, 1L);
        }
        resonance.put(p.getUniqueId(), 0);
    ...
    private void shiftImBlue(Player p) { int r = rhythm.getOrDefault(p.getUniqueId(), 0); if (r < 8) { p.sendActionBar(Component.text("🌊 Need 8+ Rhythm! (" + r + ")", NamedTextColor.AQUA)); return; } p.sendActionBar(Component.text("🌊 Deep Echo!", NamedTextColor.AQUA)); for (int i = 0; i < 3; i++) { final int wi = i; new BukkitRunnable() { @Override public void run() { if (!p.isOnline()) return; p.getLocation().getNearbyLivingEntities((wi + 1) * 4.0).stream().limit(6).forEach(le -> { if (le != p) { le.damage(12 + (wi * 4.0), p); kb(p, le, 1.5, 0.2); slow(le, 1, 40); } }); sound(p.getLocation(), Sound.BLOCK_BUBBLE_COLUMN_WHIRLPOOL_AMBIENT, 0.8f, 0.8f + (wi * 0.2f)); p.sendActionBar(Component.text("🌊 Wave " + (wi + 1) + "!", NamedTextColor.AQUA)); } }.runTaskLater(plugin, (long)(wi * 15)); } rhythm.put(p.getUniqueId(), 0); }
    ...
    private void startPassiveTask() {
        new BukkitRunnable() { @Override public void run() {
            for (Player p : Bukkit.getOnlinePlayers()) {
                UUID u = p.getUniqueId();
                // FIX: use cached lastWeapon — no PDC read every tick
    ...
    /** Fires action when player lands — safe online check every tick */
    ...
    /** FIX: safe direction vector — returns zero vector if same location to prevent NaN */
    ...
    private void fireDamage(LivingEntity target, Player attacker, double extraDmg, int fireTicks) {
        // FIX: never pass null to damage(double, Entity) — use damage(double) if no attacker
    ...
    private Location getSafeTeleport(Location loc) {
        ...
        if (!loc.getChunk().isLoaded()) return null; // FIX: chunk must be loaded
    ...
    /** FIX: also rejects slabs, stairs, snow — not just non-passable blocks */
    ...
    private void startBloodDot(LivingEntity t, Player p) {
        ...
        new BukkitRunnable() { int ticks = 0; @Override public void run() {
            ...
            // FIX: use damage(double) not damage(double, null)
    ...
    private void bleed(LivingEntity t, Player p, int ticks) {
        new BukkitRunnable() { int n = 0; @Override public void run() {
            ...
            // FIX: use damage(double) not damage(double, null)
            if (p != null && p.isOnline()) t.damage(1.5, p); else t.damage(1.5);
    ...
    private void spawnBloodStakes(Player p) {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            final Vector fd = dir.clone();
            new BukkitRunnable() { int t = 0; Location pos = p.getLocation().clone().add(fd); @Override public void run() {
    ...
    private void spawnCryptServant(Player p, Location loc) {
        if (loc.getWorld() == null) return; // FIX: null world check
        Zombie z = (Zombie) loc.getWorld().spawnEntity(loc, EntityType.ZOMBIE);
        z.setCustomName("§cCrypt Servant"); z.setCustomNameVisible(true);
    ...
    private void spawnIceExplosions(Player p, int stage) {
        ...
        for (int i = 0; i < count; i++) {
            final double ox = (rng.nextDouble() - 0.5) * radius * 2, oz = (rng.nextDouble() - 0.5) * radius * 2;
            new BukkitRunnable() { @Override public void run() {
                if (!p.isOnline()) return;
                Location eLoc = p.getLocation().add(ox, 0, oz);
                new BukkitRunnable() { @Override public void run() {
                    if (eLoc.getWorld() == null) return;
    ...
    private void startChainPull(Player p, LivingEntity target) {
        ...
        new BukkitRunnable() { int t = 0; @Override public void run() {
            ...
            if (target.getLocation().distanceSquared(anchor) > 16) { Vector pull = safeDirTo(anchor, target.getLocation()).multiply(2); target.setVelocity(pull); }
